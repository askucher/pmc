...
pub fn native_processes() -> Result<Vec<NativeProcess>, String> {
    #[cfg(target_os = "macos")]
    {
        ...
        // Allocate buffer with extra headroom to handle race condition
        // (new processes may appear between the size query and the fill call)
        let num_processes = size / mem::size_of::<KinfoProc>() + 64;
        let mut processes_buf: Vec<KinfoProc> = Vec::with_capacity(num_processes);

        // Pass the buffer's actual capacity so sysctl won't write past it
        let mut buf_size = num_processes * mem::size_of::<KinfoProc>();
        ...
        let actual_count = buf_size / mem::size_of::<KinfoProc>();
        unsafe {
            processes_buf.set_len(actual_count);
        }
