...
use crate::{
    cli::{Args, Item, Validatable, internal::Internal},
    globals::defaults,
};
...
enum Commands {
    ...
    List {
        /// Filter (use "all" to include remote servers)
        #[clap(value_parser = cli::validate::<Item>)]
        item: Option<Item>,
        /// Format output
        #[arg(long, default_value_t = string!("default"))]
...
fn main() {
    ...
    match &cli.command {
        ...
        Commands::List {
            ...
            let server_name = match item {
                Some(i) if i.get_string() == Some("all") => string!("all"),
                _ => defaults(server),
            };
            Internal::list(format, &server_name)
        }
        Commands::Logs {
            item,
            lines,
